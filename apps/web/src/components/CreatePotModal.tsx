...
export default function CreatePotModal({ open, onClose }: CreatePotModalProps) {
  ...
  const { createPot, initializing } = useUniversalAccount();
  ...
  const handleCreate = async () => {
    if (!title.trim() || !goalAmount || parseFloat(goalAmount) <= 0 || !deadline) return;
    setSending(true);
    setError("");

    try {
      await createPot({ title: title.trim(), description: description.trim(), goalAmount, deadline });
      setTitle("");
      setDescription("");
      setGoalAmount("");
      setDeadline("");
    ...
    <div className="fixed inset-0 z-[100] flex items-center justify-center p-4">
      ...
      <div className="relative bg-surface border border-border rounded-2xl w-full max-w-lg max-h-[90vh] overflow-y-auto shadow-2xl shadow-black/40">
        ...
        <div className="p-6 space-y-5">
          ...
          <div>
            <label className="block text-sm text-muted mb-1.5">Goal amount (USDC)</label>
            <input
              type="number"
              min="0"
              step="0.01"
          ...
          {!address && (
            <p className="text-sm text-accent bg-accent/10 border border-accent/20 rounded-lg px-4 py-2">
              Connect your wallet first to create a pot
            </p>
          )}
        </div>
        ...
        <div className="flex items-center justify-end gap-3 p-6 pt-0 border-t border-border">
          ...
          <button
            ...
            disabled={!title.trim() || !goalAmount || parseFloat(goalAmount) <= 0 || !deadline || !address || sending || initializing}
